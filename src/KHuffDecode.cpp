...
typedef struct {
	...
	u_char  sym;
	u_char  ending;
} kgl_http_v2_huff_decode_code_t;


static kgl_http_v2_huff_decode_code_t  kgl_http_v2_huff_decode_codes[256][16] =
{
	/* 0 */
	{
...
static inline bool kgl_http_v2_huff_decode_bits(u_char *state, u_char *ending, uintptr_t bits,u_char **dst)
{
	kgl_http_v2_huff_decode_code_t  code;

	code = kgl_http_v2_huff_decode_codes[*state][bits];
...
bool kgl_http_v2_huff_decode(u_char *state, u_char *src, size_t len, u_char **dst,uintptr_t last)
{
	...
	while (src != end) {
		...
		if (!kgl_http_v2_huff_decode_bits(state, &ending, ch >> 4, dst))
		{
			klog(KLOG_DEBUG,"http2 huffman decoding error at state %d: "
				"bad code 0x%Xd", *state, ch >> 4);
			return false;
		}

		if (!kgl_http_v2_huff_decode_bits(state, &ending, ch & 0xf, dst))
		{
			klog(KLOG_DEBUG, "http2 huffman decoding error at state %d: "
